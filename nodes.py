...
要求 ComfyUI 版本需包含 post_input patch 支持（qwen_image/model.py _forward 中）。
"""
...
class StretchRefRoPE:
        ...
        return {
            "required": {
                ...
                "stretch_indices": ("STRING", {
                    "default": "1",
                    "tooltip": (
                        "要拉伸的 ref_latent 索引，从 1 开始，对应 image1/image2/image3 顺序"
                        "（跳过未连接的）。多个用逗号分隔如 \"1,3\"。"
                        "填 \"-1\" 或留空则拉伸全部。填 \"0\" 禁用拉伸。"
                    ),
                }),
            },
        }
    ...
    def apply(self, model, stretch_indices="1"):
        # 解析要拉伸的索引（1-based → 0-based）
        # "0" = 禁用, "-1" 或 "" = 拉伸全部, 其他数字 = 指定索引
        ...
        def stretch_ref_rope_patch(patch_input):
            ...
            if ref_num_tokens is None or len(ref_num_tokens) == 0:
                if should_log:
                    print("[StretchRefRoPE] no ref_num_tokens, skipping")
                    _debug_counter += 1
            ...
            if should_log:
                print(f"[StretchRefRoPE] output h=[{out_h_min:.1f}, {out_h_max:.1f}], "
                      f"w=[{out_w_min:.1f}, {out_w_max:.1f}], refs={len(ref_num_tokens)}")
            ...
            for ref_idx, ref_tokens in enumerate(ref_num_tokens):
                ...
                if should_log:
                    status = []
                    if h_stretched:
                        status.append(f"h×{out_h_span/ref_h_span:.1f}")
                    if w_stretched:
                        status.append(f"w×{out_w_span/ref_w_span:.1f}")
                    if status:
                        print(f"[StretchRefRoPE] ref[{ref_idx}]: {ref_tokens}tok, stretched {', '.join(status)}")
                    else:
                        print(f"[StretchRefRoPE] ref[{ref_idx}]: {ref_tokens}tok, no stretch needed")
